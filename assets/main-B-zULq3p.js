(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))t(e);new MutationObserver(e=>{for(const r of e)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function s(e){const r={};return e.integrity&&(r.integrity=e.integrity),e.referrerPolicy&&(r.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?r.credentials="include":e.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function t(e){if(e.ep)return;e.ep=!0;const r=s(e);fetch(e.href,r)}})();class J{constructor(){this.routes={},this.currentStage=null,this.appContainer=document.getElementById("app")}register(n,s){this.routes[n]=s}navigateTo(n,s={}){if(!this.routes[n]){console.error(`Маршрут "${n}" не знайдено.`),n!=="welcome"&&(console.warn("Redirecting to welcome screen..."),this.navigateTo("welcome"));return}if(console.log(`Navigating to: ${n}`),this.appContainer){this.appContainer.innerHTML="";try{this.routes[n](this,s),this.currentStage=n,window.scrollTo(0,0)}catch(t){console.error(`Error initializing stage "${n}":`,t),this.appContainer.innerHTML='<div class="p-4 text-center text-red-500">Сталася помилка при завантаженні екрану. Будь ласка, оновіть сторінку.</div>'}}else console.error("CRITICAL: App container #app not found in DOM!")}}const C=new J;class G{constructor(){this.storageKey="destinyUser",this.data=this.load()}load(){try{const n=sessionStorage.getItem(this.storageKey);return n?JSON.parse(n):{}}catch{return console.warn("SessionStorage disabled or inaccessible"),{}}}save(){try{sessionStorage.setItem(this.storageKey,JSON.stringify(this.data))}catch{console.warn("Failed to save to SessionStorage")}}get(n){return this.data[n]}set(n,s){this.data[n]=s,this.save()}clear(){this.data={},sessionStorage.removeItem(this.storageKey)}}const d=new G;function j(){if(document.getElementById("global-info-modal"))return;document.body.insertAdjacentHTML("beforeend",`
    ...
    `);const n=document.getElementById("global-info-modal"),s=document.getElementById("global-modal-close"),t=()=>{n.style.display="none"};s.addEventListener("click",t),n.addEventListener("click",e=>{e.target===n&&t()})}function $(o,n){j();const s=document.getElementById("global-info-modal"),t=document.getElementById("global-modal-title"),e=document.getElementById("global-modal-message");s&&t&&e?(t.innerText=o,e.innerHTML=n,s.style.display="flex"):alert(`${o}

${n}`)}const K=`<section id="landing-step" class="funnel-step active space-y-8 text-center flex flex-col justify-between" style="min-height: calc(100vh - 80px);">
...
</div>`;let M=null;function _(){return window.CircularNatalHoroscope?Promise.resolve(!0):M?(console.log("⏳ Astro Library load request joined existing queue..."),M):(console.log("🚀 Starting Astro Library sequence..."),M=new Promise(o=>{window.module||(window.module={exports:{}}),window.exports=window.module.exports;const n=["https://unpkg.com/circular-natal-horoscope-js@1.1.0/dist/index.js","https://cdn.jsdelivr.net/npm/circular-natal-horoscope-js@1.1.0/dist/index.js","./js/astro-lib.js"];function s(t){if(t>=n.length){console.error("❌ CRITICAL: Всі джерела бібліотеки (включно з локальним) недоступні."),o(!1),M=null;return}const e=n[t],r=document.createElement("script");r.src=e,r.async=!0,r.onload=()=>{window.module&&window.module.exports&&window.module.exports.Horoscope?(window.CircularNatalHoroscope=window.module.exports,console.log(`✅ Astro Library Loaded Successfully from ${e}`),o(!0)):window.CircularNatalHoroscope?(console.log(`✅ Astro Library Loaded (Global) from ${e}`),o(!0)):(console.warn(`⚠️ Script loaded from ${e}, but exports missing.`),s(t+1))},r.onerror=()=>{console.warn(`⚠️ Failed to load from ${e}. Switching to backup source...`),s(t+1)},document.head.appendChild(r)}s(0)}),M)}function V(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=K;const s=document.getElementById("birth-form"),t=document.getElementById("birth-date"),e=document.getElementById("error-message"),r=document.getElementById("date-placeholder"),i=s.querySelector('button[type="submit"]'),m=document.getElementById("info-modal"),b=document.getElementById("legal-modal"),E=document.getElementById("open-info-modal-btn"),g=document.getElementById("close-info-modal-icon"),l=document.getElementById("close-info-modal-btn"),f=document.getElementById("close-legal-modal-icon"),h=document.getElementById("close-legal-modal-btn"),y=document.getElementById("legal-modal-body"),a=document.querySelectorAll(".legal-link[data-legal-type]");function p(){m&&(m.style.display="flex")}function c(){m&&(m.style.display="none")}function u(T){const k=document.getElementById("legal-content-"+T);k&&b&&y&&(y.innerHTML=k.innerHTML,b.style.display="flex")}function x(){b&&(b.style.display="none")}E&&E.addEventListener("click",p),g&&g.addEventListener("click",c),l&&l.addEventListener("click",c),f&&f.addEventListener("click",x),h&&h.addEventListener("click",x),m&&m.addEventListener("click",T=>{T.target===m&&c()}),b&&b.addEventListener("click",T=>{T.target===b&&x()}),a.forEach(T=>{T.addEventListener("click",k=>{const L=k.target.getAttribute("data-legal-type");c(),setTimeout(()=>{u(L)},50)})});function v(){const T=t.value;if(!T)r.innerText="Обрати дату народження",r.style.color="var(--secondary-text-color)";else{const k=T.split("-");if(k.length===3){const L=`${k[2]}.${k[1]}.${k[0]}`;r.innerText=L,r.style.color="var(--primary-text-color)"}}}function w(){t.value===""&&(console.log("Setting default date to 1995-01-01 for convenience."),t.value="1995-01-01",v())}t.addEventListener("input",v),t.addEventListener("change",v),t.addEventListener("blur",v),t.addEventListener("touchend",()=>setTimeout(v,500)),t.addEventListener("focus",w),t.addEventListener("click",w),t.addEventListener("touchstart",w),v(),s.addEventListener("submit",async function(T){T.preventDefault();const k=t.value;if(k==="")e.innerText="Будь ласка, обери дату народження.",e.style.display="block";else{let I=function(S,U){S.classList.add("loading"),S.disabled=!0};var L=I;e.style.display="none",d.set("date",k),I(i),_(),o.navigateTo("loading")}})}const Y=`<section id="loading-step" class="min-h-screen flex flex-col justify-center items-center px-4 fade-in text-center relative z-10">
...
</section>`;function P(o,n,s,t=50,e=0,r=!1){return new Promise(i=>{let m=0;n&&(n.style.display="inline-block"),o.innerHTML="";function b(){m<s.length?(o.innerHTML=s.substring(0,m+1),m++,setTimeout(b,t)):setTimeout(()=>{!r&&n&&(n.style.display="none"),i()},e)}b()})}async function W(o){if(!await _())throw new Error("Astro Library not loaded");const{Origin:n,Horoscope:s,Renderer:t}=window.CircularNatalHoroscope,e=o.date.split("-"),r=parseInt(e[0]),i=parseInt(e[1])-1,m=parseInt(e[2]);let b=12,E=0;if(o.time){const c=o.time.split(":");b=parseInt(c[0]),E=parseInt(c[1])}let g=50.45,l=30.52,f="Europe/Kyiv";if(o.geo){const c=parseFloat(o.geo.latitude||o.geo.lat),u=parseFloat(o.geo.longitude||o.geo.lon);!isNaN(c)&&!isNaN(u)&&(g=c,l=u,f=o.geo.timezone||f)}let h=[],y=null,a=null;function p(c){const u=Math.floor(c),x=(c-u)*60,v=Math.floor(x),w=Math.floor((x-v)*60);return`${u}° ${v}' ${w}"`}try{const c=new n({year:r,month:i,date:m,hour:b,minute:E,latitude:g,longitude:l,timezone:f});a=new s({origin:c,houseSystem:"placidus",zodiac:"tropical"});const u=a.CelestialBodies;["sun","moon","mercury","venus","mars","jupiter","saturn","uranus","neptune","pluto","ascendant","midheaven"].forEach(v=>{let w=u[v];if(!w&&v==="ascendant"&&(w=a.Ascendant),!w&&v==="midheaven"&&(w=a.Midheaven),w){const T=w.Sign.label.toUpperCase(),k=w.ChartPosition.Ecliptic.DecimalDegrees%30,L=p(k),I=v.toUpperCase();h.push(`${I}: ${T} ${L}`)}}),console.log("Calculated Planets (DMS):",h)}catch(c){return console.error("Horoscope Calculation Failed:",c),{planets:[],chartSvg:null,houseSystem:"Error"}}try{if(a){const c=document.createElement("div");c.style.position="absolute",c.style.left="-9999px",c.style.visibility="hidden",document.body.appendChild(c),new t(a).render(c);const x=c.querySelector("svg");x&&(x.style.backgroundColor="transparent",x.querySelectorAll("line, circle, path").forEach(v=>{const w=v.getAttribute("stroke");(!w||w==="#000000"||w==="#000")&&v.setAttribute("stroke","#cda45e");const T=v.getAttribute("fill");(T==="#000000"||T==="#000")&&v.setAttribute("fill","#cda45e")}),x.querySelectorAll("text").forEach(v=>{v.style.fill="#cda45e",v.setAttribute("fill","#cda45e"),v.style.fontFamily="'Montserrat', sans-serif"}),y=c.innerHTML),document.body.removeChild(c)}}catch(c){console.warn("SVG Render Error:",c)}return{planets:h,chartSvg:y,houseSystem:"Placidus"}}const N="QUl6YVN5RFhYQkpQaE8zV2MzdFdub25TTFdhNVMwQUItZzVObVZj",D="https://sendreportemail-kpkshoor7q-ew.a.run.app",Q="https://createpdf-kpkshoor7q-ew.a.run.app",X="gemini-2.5-flash",Z=6e4;let B=null,A=null;const ee=`Ти — 'Майстер Астро-Психолог' Destiny Code.
...
`;function se(){try{return!N||N.includes("ВСТАВ_СЮДИ")?null:atob(N)}catch{return console.error("Key decoding failed"),null}}function H(){console.log("🔥 Warming up PDF backend..."),fetch(Q,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({warmup:!0})}).catch(()=>{})}async function z(o){if(B)return console.log("⚠️ Background generation already running."),B;console.log("🚀 Starting background generation (PRE-FETCH)...");let n="";const s={...o};try{const e=await W(o);e&&e.planets&&(n=`== Технічні Астрологічні Дані ==
${e.planets.join(`
`)}`,s.planets=e.planets,d.set("planets",e.planets))}catch(e){console.warn("Local calc skipped",e)}const t=`Дата: ${o.date}
Час: ${o.time}
Місто: ${o.city}
${n}`;return B=R(ne,t).then(e=>{const r=JSON.parse(e);return A={data:r,enrichedUserData:s},console.log("✅ Background generation finished!"),r}).catch(e=>{throw console.error("Background generation failed:",e),B=null,e}),B}async function ae(o,n){let s=null,t=o;try{return A?(console.log("⚡️ Using cached report data (Instant Load)"),s=A.data,t=A.enrichedUserData):B?(console.log("⏳ Waiting for background generation to finish..."),s=await B,t=d.get("planets")?{...o,planets:d.get("planets")}:o):(console.log("🐌 Cold start generation (No pre-fetch)"),s=await z(o),t=d.get("planets")?{...o,planets:d.get("planets")}:o),n&&n.includes("@")&&(console.log("📧 Sending email/PDF request to backend..."),fetch(D,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userEmail:n,reportHtml:JSON.stringify(s),reportHtml:JSON.stringify(s),reportTitle:"Твій Повний Аналіз",reportType:"main",userData:t})}).catch(e=>console.error("Background Email Error:",e))),s}catch(e){return console.error("Generate Full Report Error:",e),e.message==="Timeout"?{error:!0,type:"timeout",message:"Час очікування вичерпано."}:{error:!0,message:"Не вдалося згенерувати звіт."}}}async function R(o,n){const s=se();if(!s)return'{"error": "config_error"}';const t=`${o}

Вхідні дані:
${n}`,e=`https://generativelanguage.googleapis.com/v1beta/models/${X}:generateContent?key=${s}`,r={contents:[{parts:[{text:t}]}],systemInstruction:{parts:[{text:ee}]}};let i=2e3;for(let m=0;m<3;m++){const b=new AbortController,E=setTimeout(()=>b.abort(),Z);try{const g=await fetch(e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r),signal:b.signal});if(clearTimeout(E),g.ok){const f=(await g.json()).candidates?.[0]?.content?.parts?.[0]?.text;if(f){const h=f.match(/```json\n([\s\S]*?)\n```/);return h&&h[1]?h[1]:f}}if(g.status===429||g.status>=500){await new Promise(l=>setTimeout(l,i)),i*=2;continue}throw new Error(`Google API Error: ${g.status}`)}catch(g){if(clearTimeout(E),g.name==="AbortError")throw new Error("Timeout");if(m===2)throw g;await new Promise(l=>setTimeout(l,i)),i*=2}}}async function re(o){H();try{const n=await R(te,`Дата народження: ${o}`);return JSON.parse(n)}catch{return{title:"Error",psychological_analysis:"Error"}}}async function O(o,n){const s=`Користувач: Жінка. Дата: ${o.date}. Місто: ${o.city}`,t=d.get("planets"),e=t?{...o,planets:t}:o;try{const r=await R(oe,s);return n&&n.includes("@")&&fetch(D,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userEmail:n,reportHtml:r,reportType:"upsell",userData:e})}).catch(i=>console.warn("Forecast email bg error:",i)),r}catch{return null}}async function le(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=Y;const s=document.getElementById("loading-text"),t=document.getElementById("loading-cursor"),e=d.get("date");H();let r=!1;const i=re(e).then(E=>(d.set("freeReport",E),r=!0,E)).catch(E=>(console.error("API Error:",E),{error:!0,title:"❌ Помилка Аналізу",psychological_analysis:"<p>На жаль, сталася помилка під час обробки відповіді від ШІ.</p>"})),m=[{text:"З'єднуюсь з ефемеридами NASA...",pause:1e3},{text:"Аналізую положення планет...",pause:1200},{text:"Зчитую твій енергетичний код...",pause:1200},{text:"Розшифровую кармічні вузли...",pause:1500},{text:"Будую твою натальну карту...",pause:1500},{text:"Приготуйся дізнатись, наскільки ти ахуєнна 😈",pause:2e3,final:!0}],b=(async()=>{for(let E=0;E<m.length;E++){const g=m[E];let l=g.pause;r&&!g.final&&(l=600),await P(s,t,g.text,50,l,g.final),g.final}t&&(t.style.display="none")})();await Promise.all([b,i]),o.navigateTo("result")}const ie=`<!-- 🔥 FIX: Додано клас 'active' для відображення -->
...
</section>`;function ce(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=ie;const s=document.getElementById("result-title"),t=document.getElementById("free-report-title"),e=document.getElementById("free-report-text"),r=document.getElementById("upgrade-button"),i=d.get("freeReport");if(!i){o.navigateTo("welcome");return}let m="";i.psychological_analysis?m=i.psychological_analysis.replace(/\*\*(.*?)\*\*/g,'<strong style="color: var(--primary-text-color);">$1</strong>').replace(/\\n/g,"<br>"):m="<p>Дані відсутні.</p>",s.innerText="Аналіз твоєї особистості",t.innerHTML=i.title||"Результат",e.innerHTML=m,r.addEventListener("click",()=>{o.navigateTo("premium-data")})}const de=`<!-- 🔥 UPDATE: Видалено центрування (margin: auto) для верху сторінки, як в моноліті -->
...
`;async function ye(o,n){const s={contents:[{parts:[{text:n}]}],systemInstruction:{parts:[{text:o}]}};try{const t=await fetch(pe,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({payload:s,modelName:ue})});if(t.ok){const r=(await t.json()).candidates?.[0];if(r&&r.content?.parts?.[0]?.text)return r.content.parts[0].text}return console.warn("Geo API Warning:",t.status),null}catch(t){return console.error("Geo API Network Error:",t),null}}async function F(o){const n=`Місто: ${o}`;try{const s=await ye(me,n);if(!s)return{error:"network_failure"};const t=s.replace(/```json\n?([\s\S]*?)\n?```/g,"$1").trim(),e=JSON.parse(t);return e&&typeof e.lat=="number"&&typeof e.lon=="number"?(console.log(`Geocoding success for ${o}:`,e),{latitude:e.lat,longitude:e.lon,timezone:e.timezone,corrected_name:e.corrected_name,error:null}):e&&e.error?(console.warn(`Geocoding failed for ${o}:`,e.error),{error:e.error}):{error:"parse_error"}}catch(s){return console.error(`Geocoding failed for ${o}:`,s),{error:"network_failure"}}}function ge(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=de;const s=document.getElementById("birth-time"),t=document.getElementById("time-placeholder"),e=s.closest(".input-field"),r=document.getElementById("time-error-message"),i=document.getElementById("birth-city"),m=document.getElementById("city-error-message"),b=document.getElementById("city-info-message"),E=document.getElementById("continue-to-paywall-button"),g=document.getElementById("skip-button");function l(){!s||!t||(s.value?(t.innerText=s.value,t.style.color="var(--primary-text-color)",e&&e.classList.remove("input-error"),r&&(r.style.display="none")):(t.innerText="Обери час",t.style.color="var(--secondary-text-color)"))}s.addEventListener("input",l),s.addEventListener("change",l),s.addEventListener("blur",l),l(),i.addEventListener("input",()=>{m.style.display="none",b.style.display="none",i.classList.remove("input-error")});function f(y,a){y&&(a?(y.classList.add("loading"),y.disabled=!0):(y.classList.remove("loading"),y.disabled=!1))}function h(y,a){y==="ambiguous"?m.innerText=`Місто "${a}" знайдено в кількох місцях. Будь ласка, уточни, додавши країну.`:m.innerText=`Не можемо знайти місто "${a}". Перевірте назву.`,m.style.display="block",i.classList.add("input-error")}E.addEventListener("click",async()=>{const y=s.value;let a=i.value.trim();const p=a;let c=!1;if(m.style.display="none",r.style.display="none",e.classList.remove("input-error"),i.classList.remove("input-error"),a||(i.classList.add("input-error"),m.innerText="Будь ласка, введи місто народження.",m.style.display="block",c=!0),y||(e.classList.add("input-error"),r.style.display="block",c=!0),!a&&c){navigator.vibrate&&navigator.vibrate(50);return}f(E,!0);const u=await F(a);let x=null;if(u&&u.latitude?(u.corrected_name&&(i.value=u.corrected_name,a=u.corrected_name,p.toLowerCase()!==u.corrected_name.toLowerCase()&&(x=`Ми уточнили: ${u.corrected_name} 😉`)),d.set("geo",{latitude:u.latitude||u.lat,longitude:u.longitude||u.lon,timezone:u.timezone}),d.set("city",u.corrected_name)):u&&u.error==="ambiguous"?(h("ambiguous",a),c=!0):(h("not_found",a),c=!0),x?(b.innerText=x,b.style.display="block"):b.style.display="none",f(E,!1),c){navigator.vibrate&&navigator.vibrate(50);return}d.set("time",y),setTimeout(()=>{o.navigateTo("paywall")},x?1200:0)}),g.addEventListener("click",async()=>{let y=i.value.trim();const a=y;if(m.style.display="none",r.style.display="none",i.classList.remove("input-error"),e.classList.remove("input-error"),!y){i.classList.add("input-error"),m.innerText="Будь ласка, введи місто, навіть якщо не знаєш часу.",m.style.display="block",navigator.vibrate&&navigator.vibrate(50);return}f(g,!0);const p=await F(y);let c=null,u=!1;if(p&&p.latitude?(p.corrected_name&&(i.value=p.corrected_name,y=p.corrected_name,a.toLowerCase()!==p.corrected_name.toLowerCase()&&(c=`Ми уточнили: ${p.corrected_name} 😉`)),d.set("geo",{latitude:p.latitude||p.lat,longitude:p.longitude||p.lon,timezone:p.timezone}),d.set("city",p.corrected_name),d.set("time","")):p&&p.error==="ambiguous"?(h("ambiguous",y),u=!0):(h("not_found",y),u=!0),c&&(b.innerText=c,b.style.display="block"),f(g,!1),u){navigator.vibrate&&navigator.vibrate(50);return}console.log("User skipped time, but city is valid."),setTimeout(()=>{o.navigateTo("paywall")},c?1200:0)})}const fe=`<section id="final-paywall-step" class="funnel-step active space-y-6">
    ...
    <div id="paywall-popup" class="modal-overlay">
...
</section>`;async function q(o){if(await _(),!window.CircularNatalHoroscope)return`
        ...
        `;const{Origin:n,Horoscope:s,Renderer:t}=window.CircularNatalHoroscope;let e=o.geo;if(e||(e={latitude:50.45,longitude:30.52,timezone:"Europe/Kyiv"}),!o.date)return"";try{let a=function(x,v){let w=y[x];if(!w&&x==="ascendant"&&(w=h.Ascendant),!w&&x==="midheaven"&&(w=h.Midheaven),w){const T=w.Sign.label,k=w.ChartPosition.Ecliptic.DecimalDegrees%30,L=Math.floor(k),I=(k-L)*60,S=Math.floor(I),U=Math.round((I-S)*60);return`
                    <div class="astro-data-item">
                        <div class="astro-label-row">
                            <span class="astro-planet-name">${v}:</span>
                             <span class="astro-sign-name">${T}</span>
                        </div>
                        <div class="astro-coords-row">${L}° ${S}' ${U}"</div>
                    </div>
                `}return`<div class="astro-data-item"><strong>${v}:</strong> n/a</div>`};var r=a;const i=o.date.split("-"),m=parseInt(i[0]),b=parseInt(i[1])-1,E=parseInt(i[2]);let g=12,l=0;if(o.time){const x=o.time.split(":");g=parseInt(x[0]),l=parseInt(x[1])}const f=new n({year:m,month:b,date:E,hour:g,minute:l,latitude:parseFloat(e.latitude),longitude:parseFloat(e.longitude),timezone:e.timezone}),h=new s({origin:f,houseSystem:"placidus",zodiac:"tropical"}),y=h.CelestialBodies,p=[];p.push(a("sun","Сонце")),p.push(a("moon","Місяць")),p.push(a("ascendant","ASC")),p.push(a("venus","Венера")),p.push(a("mars","Марс")),p.push(a("jupiter","Юпітер"));let c="";const u=document.createElement("div");u.style.position="absolute",u.style.left="-9999px",u.style.width="600px",u.style.height="600px",document.body.appendChild(u);try{new t(h).render(u);const v=u.querySelector("svg");v&&(v.style.backgroundColor="transparent",v.querySelectorAll("line, circle, path").forEach(k=>{const L=k.getAttribute("stroke");(!L||L==="#000000"||L==="#000"||L==="black")&&(k.setAttribute("stroke","#cda45e"),k.setAttribute("stroke-width","1.5"))}),v.querySelectorAll("text").forEach(k=>{k.setAttribute("fill","#cda45e"),k.style.fill="#cda45e",k.style.fontFamily="'Montserrat', sans-serif",k.style.fontWeight="500"}),c=`
                    <div class="astro-chart-preview">
                        ${u.innerHTML}
                    </div>
                `)}catch(x){console.warn("Chart Render Error:",x)}return document.body.removeChild(u),`
        ...
        `}catch(i){return console.error("Fingerprint render error:",i),`<div class="astro-data-box"><p class="text-xs text-red-400 text-center">Помилка розрахунку: ${i.message}</p></div>`}}function be(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=fe;const s=document.getElementById("paywall-timer"),t=document.getElementById("paywall-astro-data"),e=document.getElementById("final-checkout-button"),r=document.getElementById("paywall-popup"),i=document.getElementById("popup-title"),m=document.getElementById("popup-text"),b=document.getElementById("popup-checkout-btn"),E=document.getElementById("popup-close-btn");H(),window.showPaywallPopup=function(y,a){r&&i&&m&&(i.innerText=y,m.innerText=a,r.style.display="flex")},E&&E.addEventListener("click",()=>{r.style.display="none"}),b&&b.addEventListener("click",()=>{r.style.display="none",h(e)}),r&&r.addEventListener("click",y=>{y.target===r&&(r.style.display="none")});const g={date:d.get("date"),time:d.get("time"),city:d.get("city"),geo:d.get("geo")};t&&q(g).then(y=>{y?(t.innerHTML=y,t.style.display="block"):t.style.display="none"}),window.paywallInterval&&clearInterval(window.paywallInterval);let l=420;function f(){if(!s)return;const y=Math.floor(l/60),a=l%60;s.textContent=`${y<10?"0":""}${y}:${a<10?"0":""}${a}`,--l<0&&(l=0,clearInterval(window.paywallInterval))}f(),window.paywallInterval=setInterval(f,1e3);async function h(y){y.classList.add("loading"),y.disabled=!0;try{sessionStorage.setItem("destinyCodeData",JSON.stringify(g))}catch(p){console.error("Storage error:",p)}console.log("Simulating payment processing..."),await new Promise(p=>setTimeout(p,2500)),console.log("Payment simulation successful."),clearInterval(window.paywallInterval),d.set("isPaid",!0),z(g);const a=new URL(window.location);a.searchParams.set("payment","success"),window.history.pushState({},"",a),o.navigateTo("success")}e&&e.addEventListener("click",()=>h(e))}const ve=`<section id="success-step" class="funnel-step active space-y-6">
...
</section>`;function he(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=ve;const s=document.getElementById("email-form"),t=document.getElementById("user-email"),e=document.getElementById("main-report-btn"),r=document.getElementById("ltv-upsell-box"),i=document.getElementById("ltv-upsell-btn"),m=document.getElementById("upsell-email-modal"),b=document.getElementById("upsell-email-form"),E=document.getElementById("upsell-email-input");d.get("email")&&(t.value=d.get("email"));function g(){if(e){e.classList.remove("btn-primary"),e.classList.add("btn-gold-purple");const l=e.querySelector(".btn-text");l&&(l.innerText="Надіслати мені звіт + Прогноз")}}i.addEventListener("click",async()=>{const l=i,f=l.querySelector(".btn-text").innerText;l.classList.add("loading"),l.disabled=!0,l.querySelector(".btn-text").innerText="Обробка платежу...";try{console.log("Upsell payment initiated..."),await new Promise(y=>setTimeout(y,2e3)),console.log("Upsell Payment successful."),d.set("hasPaidUpsell",!0),l.classList.remove("loading"),l.querySelector(".btn-text").innerText="Оплачено! ✅",l.style.opacity="0.7",g(),setTimeout(()=>{r.style.display="none"},1500);const h=t.value;if(h&&h.includes("@")){const y={date:d.get("date"),time:d.get("time"),city:d.get("city"),geo:d.get("geo")};O(y,h),alert("Прогноз оплачено! Натисніть 'Надіслати мені звіт', щоб завершити.")}else m.style.display="flex"}catch(h){console.error("Upsell Error:",h),l.classList.remove("loading"),l.disabled=!1,l.querySelector(".btn-text").innerText=f,alert("Помилка оплати. Спробуйте ще раз.")}}),b.addEventListener("submit",l=>{l.preventDefault();const f=E.value;if(f){m.style.display="none",t.value=f,d.set("email",f);const h={date:d.get("date"),time:d.get("time"),city:d.get("city"),geo:d.get("geo")};O(h,f),g()}}),s.addEventListener("submit",l=>{l.preventDefault();const f=t.value;f&&(d.set("email",f),o.navigateTo("generation"))})}const xe=`<!-- 🔥 UPDATE: Центрування анімації звіту за допомогою CSS класу -->
<section id="generation-step" class="funnel-step active step-centered text-center" style="margin-top: auto; margin-bottom: auto;">
...
</section>`;async function we(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=xe;const s=document.getElementById("report-loading-text"),t=document.getElementById("report-cursor"),e={date:d.get("date"),time:d.get("time"),city:d.get("city"),geo:d.get("geo")},r=d.get("email");d.get("hasPaidUpsell")&&O(e,r).catch(l=>console.warn("Forecast bg error:",l));let i=!1;const m=ae(e,r).then(l=>l&&!l.error?(d.set("fullReport",l),i=!0,{success:!0}):{success:!1,message:l.message,type:l.type}).catch(l=>(console.error("API Network Error:",l),{success:!1,message:"Проблема з мережею"})),b=50,E=[{text:"✨ Аналізую Ядро твоєї Особистості",pause:1e3},{text:"❤️‍🔥 Розшифровую твої сценарії Кохання",pause:1e3},{text:"👑 Шукаю, де приховані твої Гроші",pause:1e3},{text:"🔮 Вивчаю твої Кармічні Уроки",pause:1e3},{text:"⚡️ Формую структуру твого звіту",pause:500}];for(const l of E)await P(s,t,l.text,b,l.pause,!1);if(!i){const l=["✍️ Дописую розділ про майбутнє...","🎨 Оформлюю твої таблиці...","✨ Додаю останні штрихи...","🚀 Майже готово..."];let f=0;for(;!i;){await P(s,t,l[f],b,0,!1);for(let h=0;h<20&&!i;h++)await new Promise(y=>setTimeout(y,100));f=(f+1)%l.length}}await P(s,t,"✅ Звіт готовий!",30,500,!0);const g=await m;if(g.success)o.navigateTo("premium-result");else{let l="З'єднання перервано";g.type==="timeout"?l="Сервер прогрівається. Спробуйте ще раз.":g.message&&(l=g.message),s.innerHTML=`<span style="color: #ef4444; font-size: 0.9em;">⚠️ ${l}</span>`,t&&(t.style.display="none");const f=document.createElement("button");f.className="btn btn-primary mt-4",f.innerText="Натисніть для повтору ↻",f.onclick=()=>o.navigateTo("generation"),document.getElementById("report-typing-container").appendChild(f)}}const Ee=`<section id="premium-result-step" class="funnel-step active space-y-6">
    ...
    <div id="late-upsell-modal" class="modal-overlay">
        <div class="modal-content" style="width: 480px; max-width: 95%;">
...
</section>`;function ke(o){const n=document.getElementById("app");n.classList.add("funnel-container"),n.innerHTML=Ee;const s=document.getElementById("full-report-content"),t=document.getElementById("report-actions-container"),e=document.getElementById("late-upsell-modal"),r=document.getElementById("close-late-upsell"),i=document.getElementById("late-upsell-btn"),m=d.get("fullReport"),b={date:d.get("date"),time:d.get("time"),city:d.get("city"),geo:d.get("geo"),planets:d.get("planets")||[]};d.get("hasPaidUpsell");const E=d.get("email");async function g(){if(!m||!m.sections){s.innerHTML=`
            ...
            `,document.getElementById("refresh-report-btn").onclick=()=>window.location.reload();return}let a="";for(const u of m.sections){const x=u.analysis_text.split(`
`).filter(w=>w.trim().length>0).map(w=>`<p>${w.trim().replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>")}</p>`).join(""),v=u.practical_advice.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\\n/g,"<br>");a+=`
                <div class="report-section mb-6">
                    ...
                    <div class="report-advice mt-4">
                        ...
                        <p class="mt-2" style="color: var(--primary-text-color); opacity: 0.9;">${v}</p>
                    </div>
                </div>
            `}const p=await q(b),c=a+p;s.innerHTML=c,l()}function l(){t.innerHTML="";const a=document.createElement("button");if(a.className="btn btn-secondary",a.id="download-pdf-btn",a.innerHTML='<span class="btn-text">Завантажити PDF</span><span class="btn-spinner"></span>',a.onclick=()=>f(a,s.innerHTML),t.appendChild(a),d.get("hasPaidUpsell")){const p=document.createElement("button");p.className="btn btn-secondary",p.style.marginTop="10px",p.innerHTML='<span class="btn-text">Спробувати ще (Почати знову)</span>',p.onclick=()=>{if(confirm("Ви впевнені? Це очистить поточний звіт.")){d.clear();const c=window.location.pathname;window.history.replaceState({},document.title,c),window.location.href=c}},t.appendChild(p)}else{const p=document.createElement("button");p.className="btn btn-violet",p.style.marginTop="10px",p.innerHTML='<span class="btn-text">Отримати прогноз на рік</span>',p.onclick=()=>{e.style.display="flex"},t.appendChild(p)}}async function f(a,p){h(a,!0);const c="https://createpdf-kpkshoor7q-ew.a.run.app";try{const u=document.createElement("div");u.innerHTML=p;const x=u.querySelector(".astro-data-box");x&&x.remove();const v=u.innerHTML,w=Array.from(document.styleSheets).map(L=>{try{return Array.from(L.cssRules).map(I=>I.cssText).join(`
`)}catch{return""}}).join(`
`);console.log("Sending PDF Request with Planets:",b.planets);const T=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reportHtml:v,reportStyles:w,userData:b,reportType:"main"})});if(!T.ok)throw new Error("Backend error");const k=await T.json();if(k.success&&k.pdfBase64){const L=y(k.pdfBase64,"application/pdf"),I=URL.createObjectURL(L),S=document.createElement("a");S.href=I,S.download=k.filename||"DestinyCode_Report.pdf",document.body.appendChild(S),S.click(),document.body.removeChild(S),URL.revokeObjectURL(I)}else throw new Error("Invalid response")}catch(u){console.error(u),$("Помилка","Не вдалося завантажити PDF. Спробуйте ще раз.")}finally{h(a,!1)}}r.addEventListener("click",()=>{e.style.display="none"}),i.addEventListener("click",async()=>{const a=i,p=a.querySelector(".btn-text").innerText;a.classList.add("loading"),a.disabled=!0,a.querySelector(".btn-text").innerText="Обробка...";try{await new Promise(c=>setTimeout(c,2e3)),d.set("hasPaidUpsell",!0),E&&O(b,E),a.classList.remove("loading"),a.querySelector(".btn-text").innerText="Оплачено! ✅",setTimeout(()=>{e.style.display="none",l(),$("Успіх!","Прогноз оплачено і відправлено на пошту!")},1e3)}catch{a.classList.remove("loading"),a.disabled=!1,a.querySelector(".btn-text").innerText=p,$("Помилка","Не вдалося провести оплату.")}});function h(a,p){p?(a.classList.add("loading"),a.disabled=!0):(a.classList.remove("loading"),a.disabled=!1)}function y(a,p){const c=atob(a),u=[];for(let x=0;x<c.length;x+=512){const v=c.slice(x,x+512),w=new Array(v.length);for(let T=0;T<v.length;T++)w[T]=v.charCodeAt(T);u.push(new Uint8Array(w))}return new Blob(u,{type:p})}g()}C.register("welcome",V);C.register("loading",le);C.register("result",ce);C.register("premium-data",ge);C.register("paywall",be);C.register("success",he);C.register("generation",we);C.register("premium-result",ke);document.addEventListener("DOMContentLoaded",()=>{j();const n=new URLSearchParams(window.location.search).get("payment"),s=sessionStorage.getItem("destinyUser");if(s)try{const t=JSON.parse(s);d.set("date",t.date),d.set("time",t.time),d.set("city",t.city),d.set("geo",t.geo),console.log("Session restored")}catch(t){console.error("Failed to restore session:",t)}n==="success"?(window.history.replaceState({},document.title,window.location.pathname),C.navigateTo("success")):C.navigateTo("welcome")});
